...
bot = telebot.TeleBot('7083053513:AAEKzeYBIy274514QEmMi2Xvn1esjQY3gZk')
...
def handle_attack(message):
    ...
    if user_id in allowed_user_ids:
        if attack_running:
            response = "An attack is already running. Please wait for it to finish before starting a new one."
            bot.reply_to(message, response)
            return
        ...
        if len(command) == 4:  # Updated to accept target, port, and time
            ...
            if time > 180:
                ...
                try:
                    ...
                    full_command = f"./11124 {target} {port} {time} 800"
